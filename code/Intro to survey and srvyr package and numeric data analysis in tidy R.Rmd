...
```{r explore, echo = T}
### We can use the function summary() to get a quick summary of this variable. Note there are missing values (NA's)
NHANESraw |>
  pull(alcohol_day) |>
  summary()
...
nhanes_design |>
  ...
  summarise(mean = survey_mean(alcohol_day, na.rm = T, vartype = "ci"), 
            median = survey_median(alcohol_day, na.rm = T, vartype = "ci"), 
            quantile = survey_quantile(alcohol_day, c(0.25, 0.75), na.rm = T, vartype = "ci")) |>
...
```{r ttest, echo = T}
svyttest(alcohol_day ~ gender, design = nhanes_design) 
...
```{r linear, echo = T}
alc_model <- svyglm(alcohol_day ~ age, design = nhanes_design, family = gaussian) 
...
alc_model2 <- svyglm(alcohol_day ~ age + gender, design = nhanes_design, family = gaussian)
