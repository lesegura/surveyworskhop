...
Calculate the total variance by summing the sum of squares of the clustering variable and the residuals. Save the results in an object called total_var.
```{r total variance, echo = T}
### calculate total variance
total_var <- model_anova |>
  summarise(total_var = sum(sumsq))
```

The total variance is `r total_var`

Then save the variance between clusters in an object called cluster_var
```{r cluster variance, echo = T}
...
The between cluster variance is `r cluster_var`
